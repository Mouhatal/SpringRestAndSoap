...
/**
 * Performance test for the SituationCovid entity.
 */
class SituationCovidGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SituationCovid entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all situationCovids")
            .get("/services/apprest/api/situation-covids")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new situationCovid")
            .post("/services/apprest/api/situation-covids")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nbreCas":"0"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_situationCovid_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created situationCovid")
                .get("/services/apprest${new_situationCovid_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created situationCovid")
            .delete("/services/apprest${new_situationCovid_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
